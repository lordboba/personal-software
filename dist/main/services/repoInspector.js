...
export async function inspectRepo(localPath, defaultBranch) {
    ...
    if (hasPackage) {
        ...
        if (deps.vite || pkg.scripts?.dev?.includes("vite"))
            detectedStack.push("Vite");
        if (deps.electron)
            detectedStack.push("Electron");
        if (deps.react)
            detectedStack.push("React");
        if (pkg.scripts?.dev)
            devCommand = `${packageManager ?? "npm"} run dev`;
        if (pkg.scripts?.build)
            buildCommand = `${packageManager ?? "npm"} run build`;
    }
    const envTargets = [];
    if (await exists(path.join(localPath, ".env.example")))
        envTargets.push(".env");
    if (hasPackage)
        envTargets.push(".env.local");
    return {
        defaultBranch,
        packageManager,
...
export function commandsFromInspection(localPath, inspection) {
    return [
